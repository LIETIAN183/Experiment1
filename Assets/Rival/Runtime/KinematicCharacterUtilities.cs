...
namespace Rival
{
    ...
    public static class KinematicCharacterUtilities
    {
            ...
            AuthoringKinematicCharacterBody authoringProperties)
        {
            ...
            // Interpolation
            if (authoringProperties.UseInterpolation)
            {
            ...
            bool constrainRotationToGroundingUp) where T : struct, IKinematicCharacterProcessor
        {
            ...
            if (characterBody.ParentEntity != Entity.Null)
            {
                    ...
                    math.lengthsq(displacementFromParentMovement) > math.EPSILON)
                {
                    ...
                    {
                        Collider = characterPhysicsCollider.ColliderPtr,
                        Orientation = characterRotation,
                        Start = characterTranslation,
                        End = characterTranslation + (castDirection * castLength),
                    };
                    ...
                    if (FilterColliderCastHitsForMove(ref processor, ref tmpColliderCastHits, characterBody.ShouldIgnoreDynamicBodies(), characterEntity, castDirection, characterBody.ParentEntity, out ColliderCastHit closestHit, out bool foundAnyOverlaps))
                    {
                        characterTranslation += closestHit.Fraction * castLength;
                    }
                    else
                    {
            ...
            out float distanceToGround) where T : struct, IKinematicCharacterProcessor
        {
            ...
            ColliderCastInput input = new ColliderCastInput()
            {
                ...
                Orientation = characterRotation,
                Start = characterTranslation,
                End = characterTranslation + (-groundingUp * groundProbingLength),
            };
            tmpColliderCastHits.Clear();
            AllHitsCollector<ColliderCastHit> collector = new AllHitsCollector<ColliderCastHit>(1f, ref tmpColliderCastHits);
                    ...
                    if (hitBodyEntity != characterBody.ParentEntity)
                    {
                        ...
                        if (bodyHasPhysicsVelocityAndMass)
                        {
                            if (!tmpRigidbodyIndexesProcessed.Contains(characterHit.RigidBodyIndex))
                            {
                                ...
                                float selfEffectiveMass = characterBody.Mass;
                                bool hitBodyIsDynamic = false;
                                float3 bodyPointVelocity = default;
                                ...
                                float3 effectiveHitNormal = characterHit.Normal;
                                if (characterHit.WasCharacterGroundedOnHitEnter && !characterHit.IsGroundedOnHit)
                                {
                                    effectiveHitNormal = math.normalizesafe(MathUtilities.ProjectOnPlane(characterHit.Normal, groundingUp));
                                }
                                else if (characterHit.IsGroundedOnHit)
                                {
                                    effectiveHitNormal = groundingUp;
                                }
                                ...
                                if (hitBodyIsCharacter)
                                {
                                    ...
                                    bodyEffectiveMassAtPoint = modifiedMass;
                                }
                                else if (bodyHasPhysicsVelocityAndMass)
                                {
                                    Translation bodyTranslation = new Translation { Value = hitBody.WorldFromBody.pos };
                                    Rotation bodyRotation = new Rotation { Value = hitBody.WorldFromBody.rot };
                                    PhysicsVelocity bodyPhysicsVelocity = physicsVelocityFromEntity[hitBodyEntity];
                                    PhysicsMass bodyPhysicsMass = physicsMassFromEntity[hitBodyEntity];

                                    hitBodyIsDynamic = PhysicsUtilities.IsEntityDynamic(bodyHasPhysicsVelocityAndMass, bodyHasPhysicsVelocityAndMass, bodyPhysicsMass.InverseMass);
                                    bodyPointVelocity = bodyPhysicsVelocity.GetLinearVelocity(bodyPhysicsMass, bodyTranslation, bodyRotation, characterHit.Position);

                                    if (hitBodyIsDynamic)
                                    {
                                        bodyTrueMassAtPoint = 1f / PhysicsUtilities.GetInvMassAtPoint(characterHit.Position, effectiveHitNormal, hitBody, bodyPhysicsMass.InverseMass, bodyPhysicsMass.InverseInertia);

                                        float modifiedMass = 1f / bodyPhysicsMass.InverseMass;
                                        processor.OverrideDynamicHitMasses(ref selfEffectiveMass, ref modifiedMass, characterEntity, hitBodyEntity, hitBodyIndex, true);

                                        if (modifiedMass <= 0f)
                                        {
                                            bodyEffectiveMassAtPoint = 0f;
                                        }
                                        else
                                        {
                                            bodyEffectiveMassAtPoint = 1f / PhysicsUtilities.GetInvMassAtPoint(characterHit.Position, effectiveHitNormal, hitBody, 1f / modifiedMass, bodyPhysicsMass.InverseInertia);
                                        }
                                    }
                                }

                                // Prevent a grounding-reoriented normal for dynamic bodies
                                if (hitBodyIsDynamic && !characterHit.IsGroundedOnHit)
                                {
                                    effectiveHitNormal = characterHit.Normal;
                                }

                                // Calculate the ratio of the total mass that the character mass represents
                                float characterToBodyMassRatio = 1f;
                                {
                                    if (selfEffectiveMass + bodyEffectiveMassAtPoint > 0f)
                                    {
                                        characterToBodyMassRatio = selfEffectiveMass / (selfEffectiveMass + bodyEffectiveMassAtPoint);
                                    }
                                    else
                                    {
                                    ...
                                    else if (!characterBody.SimulateDynamicBody && !hitBodyIsCharacter)
                                    {
                                ...
                                characterBody.RelativeVelocity += math.projectsafe(characterHit.CharacterVelocityBeforeHit - characterHit.CharacterVelocityAfterHit, effectiveHitNormal);

                                // Calculate the required velocity changes from the collision
                                PhysicsUtilities.SolveCollisionVelocities(
                                    characterBody.RelativeVelocity + characterBody.ParentVelocity,
                                    bodyPointVelocity,
                                    effectiveHitNormal,
                                    characterToBodyMassRatio,
                                    out float3 velocityChangeOnCharacter,
                                    out float3 velocityChangeOnBody);

                                // Apply velocity change on our character
                                characterBody.RelativeVelocity += (velocityChangeOnCharacter);
                                ...
                                if (characterBody.IsGrounded && math.dot(characterBody.RelativeVelocity, characterBody.GroundHit.Normal) < -Constants.DotProductSimilarityEpsilon)
                                {
                                    characterBody.RelativeVelocity = MathUtilities.ProjectOnPlane(characterBody.RelativeVelocity, groundingUp);
                                    characterBody.RelativeVelocity = MathUtilities.ReorientVectorOnPlaneAlongDirection(characterBody.RelativeVelocity, characterBody.GroundHit.Normal, groundingUp);
                                }

                                // Apply velocity change on hit body (only if dynamic)
                                if (hitBodyIsDynamic && math.lengthsq(velocityChangeOnBody) > 0f)
                                {
                                    characterDeferredImpulsesBuffer.Add(new KinematicCharacterDeferredImpulse
                                    {
                                        OnEntity = hitBodyEntity,
                                        AtPoint = characterHit.Position,
                                        Impulse = hitBodyIsCharacter ? velocityChangeOnBody : velocityChangeOnBody * bodyTrueMassAtPoint,
                                    });
                                }
                            }
                        }
            ...
            out bool confirmedNoOverlapsOnLastMoveIteration) where T : struct, IKinematicCharacterProcessor
        {
            ...
            if (characterBody.AddCloseHitsForVelocityProjection)
            {
            ...
            if (characterBody.DetectMovementCollisions)
            {
                ...
                while (movementCastIterationsMade < characterBody.MaxContinuousCollisionsIterations && remainingMovementLength > 0f)
                {
                    ...
                    ColliderCastInput castInput = new ColliderCastInput
                    {
                        Collider = characterPhysicsCollider.ColliderPtr,
                        Orientation = characterRotation,
                        Start = castStartPosition,
                        End = castStartPosition + (castDirection * castLength),
                    };
                    tmpColliderCastHits.Clear();
                    AllHitsCollector<ColliderCastHit> collector = new AllHitsCollector<ColliderCastHit>(1f, ref tmpColliderCastHits);
                    collisionWorld.CastCollider(castInput, ref collector);
                    bool foundMovementHit = FilterColliderCastHitsForMove(ref processor, ref tmpColliderCastHits, characterBody.ShouldIgnoreDynamicBodies(), characterEntity, castDirection, Entity.Null, out ColliderCastHit closestHit, out bool foundAnyOverlaps);
            ...
            float3 groundingUp) where T : struct, IKinematicCharacterProcessor
        {
                ...
                bool projectVelocityOnHit)
            {
                ...
                {
                    // Move basec on how far the obstruction was
                    characterTranslation += decollisionDirection * closestHitDistance;

                    // Add a position displacement impulse to the decollided body, to move it away from us
            ...
            while (decollisionIterationsMade < characterBody.MaxOverlapDecollisionIterations)
            {
                ...
                ColliderDistanceInput distanceInput = new ColliderDistanceInput
                {
                    Collider = characterPhysicsCollider.ColliderPtr,
                    MaxDistance = 0f,
                    Transform = math.RigidTransform(characterRotation, characterTranslation),
                };
            ...
            out float hitDistance) where T : struct, IKinematicCharacterProcessor
        {
            ...
            ColliderCastInput castInput = new ColliderCastInput
            {
                ...
                Orientation = characterRotation,
                Start = characterTranslation,
                End = characterTranslation + (direction * length),
            };
            tmpColliderCastHits.Clear();
            AllHitsCollector<ColliderCastHit> collector = new AllHitsCollector<ColliderCastHit>(1f, ref tmpColliderCastHits);
            collisionWorld.CastCollider(castInput, ref collector);
            ...
            for (int i = 0; i < hits.Length; i++)
            {
                ...
                if (hit.Entity != ignoredEntity)
                {
                    if (hit.Entity != characterEntity)
                    {
                        if (processor.CanCollideWithHit(new BasicHit(hit)))
                        {
                            // Remember overlaps (must always include dynamic hits)
        ...
        public static JobHandle ScheduleDeferredImpulsesJob(SystemBase forSystem, EntityQuery characterQuery, JobHandle dependency, bool run = false)
        {
            KinematicCharacterDeferredImpulsesJob deferredImpulsesJob = new KinematicCharacterDeferredImpulsesJob
            {
                ...
                PhysicsMassFromEntity = forSystem.GetComponentDataFromEntity<PhysicsMass>(true),
                TranslationFromEntity = forSystem.GetComponentDataFromEntity<Translation>(false),
                RotationFromEntity = forSystem.GetComponentDataFromEntity<Rotation>(true),
            };
        ...
        public static class DefaultMethods
        {
            ...
            public static void UpdateGroundPushing(
                ref CollisionWorld collisionWorld,
                ref ComponentDataFromEntity<PhysicsMass> physicsMassFromEntity,
                ref DynamicBuffer<KinematicCharacterDeferredImpulse> characterDeferredImpulsesBuffer,
                ref KinematicCharacterBody characterBody,
                float deltaTime,
                float3 gravity,
                float forceMultiplier = 1f)
            {
                if (characterBody.IsGrounded)
                {
                    ...
                    if (groundEntity != Entity.Null && PhysicsUtilities.IsBodyDynamic(in physicsMassFromEntity, groundEntity))
                    {
                        RigidBody groundRigidBody = collisionWorld.Bodies[characterBody.GroundHit.RigidBodyIndex];

                        characterDeferredImpulsesBuffer.Add(new KinematicCharacterDeferredImpulse
                        {
                            OnEntity = groundRigidBody.Entity,
                            AtPoint = characterBody.GroundHit.Position,
                            Impulse = gravity * characterBody.Mass * deltaTime * forceMultiplier,
                        });
                    }
                }
            }
