...
namespace Rival
{
    public static class PhysicsUtilities
    {
        ...
        public static unsafe float3 GetHitFaceNormal(RigidBody hitBody, float3 surfaceNormal, ColliderKey colliderKey)
        {
            if (hitBody.Collider.Value.GetLeaf(colliderKey, out ChildCollider hitChildCollider))
            {
                ...
                if (colliderType == ColliderType.Triangle || colliderType == ColliderType.Quad)
                {
                    ...
                    float3 faceNormal = math.rotate(hitBody.WorldFromBody, localFaceNormal);
