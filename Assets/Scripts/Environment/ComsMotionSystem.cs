...
{
    public static readonly float staticFriction = 0.4f;
    ...
    protected override void OnUpdate()
    {
        ...
        .ForEach((ref PhysicsVelocity physicsVelocity, ref ComsData data, in Translation translation, in Rotation rotation, in PhysicsMass physicsMass) =>
        {
            ...
            if (math.abs(physicsVelocity.Linear.y - data.previous_VelInY) >= 4 * time)
            //math.abs(physicsVelocity.Linear.y) < threshold || translation.Value.y - data.previous_VelInY < threshold)
            {
            ...
            // if (horiAcc.Equals(float3.zero)) return;
            physicsVelocity.ApplyLinearImpulse(physicsMass, -horiAcc * 3f / physicsMass.InverseMass * time);

            data.previous_VelInY = physicsVelocity.Linear.y;
        }).ScheduleParallel();

        Entities.WithAll<cartTag>()
        .ForEach((ref PhysicsVelocity physicsVelocity, ref ComsData data, in Translation translation, in Rotation rotation, in PhysicsMass physicsMass) =>
        {
