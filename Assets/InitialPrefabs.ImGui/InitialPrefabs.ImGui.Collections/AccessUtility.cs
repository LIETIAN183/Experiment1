...
namespace InitialPrefabs.NimGui.Collections
{
    ...
    internal static class AccessUtility
    {
        ...
        [Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
        internal static void CheckExists<T>(int id, UnsafeParallelHashMap<int, T> container) where T : unmanaged
        {
        ...
        [Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
        internal static void CheckExists<T>(int id, NativeParallelHashMap<int, T> container) where T : unmanaged
        {
            if (!container.ContainsKey(id))
            {
                throw new InvalidOperationException($"ID {id} does not exist.");
